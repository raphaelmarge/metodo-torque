...
(async()=>{
 ...
 await ctx.addInitScript(registros=>{localStorage.setItem('ptdc',JSON.stringify(registros));localStorage.setItem('ptfeitos',JSON.stringify({'2026-09-07':1,'2026-09-06':1,'2026-09-05':1,'2026-08-27':1}));localStorage.setItem('pttour','{}');localStorage.setItem('ptonb','{"feito":true}');},registros);
 ...
 eq(await page.locator('#evAltK').textContent(),'Evolução','Cabeçalho identifica a área sem destacar XP');
 eq(await page.locator('#evAltN').textContent(),'1 de 3','Resumo usa a meta semanal canônica');
 ok(await page.locator('#cqTiles').isVisible()&&await page.locator('#cqGraf').isVisible(),'Constância e semanas aparecem antes das medalhas');
 ...
 ok(!await page.locator('#evCalendario').evaluate(e=>e.open)&&!await page.locator('#evReconhecimentos').evaluate(e=>e.open),'Calendário e medalhas começam recolhidos');
 ok(!await page.locator('#evXp').isVisible()&&!await page.locator('#cqGrid').isVisible()&&!await page.locator('#nvCard').isVisible(),'XP e medalhas não ocupam o resumo nem duplicam o nível');
 eq(await page.locator('#evTopo #evXp').count(),0,'Nó XP foi movido para seu detalhe');
 const ids=['cqTiles','cqGraf','mapaAno','cqGrid','cqVerMais','evTopoNv','evRing','evNvNum','evXp','evFalta','retroCard','retroShare','retroFecha','btnCardStories'];
 ...
 await page.locator('#evCalendario>summary').click();await page.locator('#mapAnt').click();eq(await page.evaluate(()=>window.__mapaMes.mes()),1,'Calendário anterior continua funcionando dentro do detalhe');
 await page.locator('#evCalendario>summary').click();await page.locator('#evCalendario>summary').click();eq(await page.evaluate(()=>window.__mapaMes.mes()),1,'Fechar calendário preserva o mês selecionado');
 await page.locator('#mapVa').click();eq(await page.evaluate(()=>window.__mapaMes.vis()),'ano','Alternância para Ano conserva o contrato');
 await page.evaluate(()=>{window.__evSub('corpo');window.__evSub('conq');});
 ok(await page.locator('#evCalendario').evaluate(e=>e.open)&&await page.locator('#mapaAnoRol').isVisible(),'Voltar ao Resumo mantém o calendário e a visão anual abertos');
 ...
 await page.locator('#evCalendario>summary').click();
 await page.locator('#evReconhecimentos>summary').click();await page.waitForFunction(()=>document.getElementById('evXp').textContent===document.getElementById('xpNum').textContent+' XP');eq(await page.locator('#evXp').textContent(),(await page.locator('#xpNum').textContent())+' XP','XP recolhido conserva o valor canônico após a animação');
 ...
 await page.evaluate(()=>{window.__evSub('corpo');window.__evSub('conq');});ok(await page.locator('#evReconhecimentos').evaluate(e=>e.open)&&await page.locator('#cqGrid>button').nth(6).isVisible(),'Navegar preserva abertura e expansão das medalhas');
 ...
 await page.locator('#evReconhecimentos>summary').click();
 ...
 ok(await page.locator('#retroCard').isVisible(),'Retrospectiva mantém sua regra de dados do mês anterior');await page.locator('#retroFecha').click();await page.evaluate(()=>{window.__evSub('corpo');window.__evSub('conq');});ok(!await page.locator('#retroCard').isVisible(),'Retrospectiva dispensada não reaparece ao trocar abas');
 ...
 for(const width of [320,390])for(const claro of [false,true]){
  await page.setViewportSize({width,height:844});await page.evaluate(claro=>{document.documentElement.classList.toggle('claro',claro);document.getElementById('evCalendario').open=false;document.getElementById('evReconhecimentos').open=false;scrollTo(0,0);},claro);await page.waitForTimeout(550);
  ...
  await page.locator('#evCalendario>summary').click();await page.locator('#mapVm').click();
