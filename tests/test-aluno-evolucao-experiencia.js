...
(async()=>{
 ...
 ok(!await page.locator('#evCalendario').evaluate(e=>e.open),'Calendário continua recolhido inicialmente');
 ...
 const ids=['cqTiles','cqGraf','mapaAno','cqGrid','cqVerMais','evTopoNv','evRing','evNvNum','evXp','evFalta','evReconhecimentos','evResumoTitulo','retroCard','retroShare','retroFecha','btnCardStories'];
 ...
 for(const width of [320,390])for(const claro of [false,true]){
  ...
  await page.evaluate(claro=>{document.documentElement.classList.toggle('claro',claro);document.getElementById('evCalendario').open=false;scrollTo(0,0);},claro);await page.waitForTimeout(550);
