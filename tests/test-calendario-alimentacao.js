...
async function mount(plan=PLAN,extra={}){
 ...
 planoApp:{'4':[{tp:'ficha',i:0,n:'Treino A',h:'18:00'}],'6':[{tp:'ficha',i:0,n:'Treino A',h:'09:00'}]},sessApp:[],wodsApp:[],cardiosApp:[],nutricaoApp:plan,...extra};
...
(async()=>{
 ...
 ok((await p.locator('#ntpResumo').textContent()).includes('refeições programadas')&&!(await p.locator('#ntpResumo').textContent()).includes('refeições registradas'),'resumo futuro distingue programação de consumo registrado');
 ...
 await p.locator('.ntp-meal-search>summary').click();await p.locator('#ntpBuscaRefeicao').fill('filtro sem resultado');
 ...
 eq(await p.locator('#ntpBuscaRefeicao').inputValue(),'','atalho do calendário limpa a busca que esconderia a refeição');
 eq(await p.locator('[data-ntp-filtro][aria-pressed=true]').getAttribute('data-ntp-filtro'),'todas','calendário restaura a lista completa antes de focar');
 eq(await p.evaluate(()=>document.activeElement.dataset.ntpMeal),'almoco','foco chega à refeição mesmo após um filtro sem resultado');
 eq(await p.evaluate(()=>JSON.parse(__calLS.getItem(window.__nutriAluno.chaves.rascunho)).titulo),'Rascunho que precisa permanecer','navegação conserva o rascunho também no armazenamento');
 ...
 const dated=await mount(PLAN,{atendimento:'online',planoDatas:{[TODAY]:[{tp:'ficha',i:0,n:'Treino por data',h:'20:30'}],[SATURDAY]:[]}});
 eq(await dated.page.evaluate(d=>window.__agItens(d).filter(i=>i.k==='treino').map(i=>({h:i.h,tit:i.tit})),TODAY),[{h:'20:30',tit:'Treino por data'}],'programação por data v823 prevalece sobre o padrão semanal');
 eq(await dated.page.evaluate(d=>window.__agItens(d).filter(i=>i.k==='treino').length,SATURDAY),0,'data de descanso explícita não recupera treino semanal');
 eq(await dated.page.evaluate(d=>window.__agItens(d).filter(i=>i.k==='alimentacao').length,TODAY),3,'programação por data conserva as refeições vigentes');
 await dated.page.evaluate(()=>window.__trocaSec('agenda'));
 ok(!await dated.page.locator('#agPede').isVisible(),'aluno online conserva a restrição de agendamento presencial v823');
 await dated.ctx.close();
