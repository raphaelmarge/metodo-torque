...
async function details(p,id){const el=p.locator(id);if(!await el.evaluate(x=>x.open))await el.locator(':scope>summary').click();}
...
async function testMain(){
  ...
  ok((await p.locator('#ntpResumo').textContent()).includes('Sem registro não significa que você não comeu'),'Resumo distingue falta de registro de não comer');
  ...
  await p.locator('[data-ntp-comi=almoco]').click();await sync(p);
  ...
  await p.locator('#ntpReceitaBusca').fill('antiga');await details(p,'.ntp-recipe');eq(await p.locator('#ntpReceitasLista [data-ntp-receita]').count(),0,'Receita legada sem nutrientes completos fica somente para consulta');
  ...
  await p.locator('[data-ntp-conversa="'+id+'"]').click();await p.waitForFunction(()=>document.getElementById('ntpComentarios').textContent.includes('Como você'));ok((await p.locator('#ntpComentarios').textContent()).includes('Refeição revisada'),'Selo da revisão corresponde à versão exata do registro');
  ...
  state.feedbackHold=true;await p.locator('[data-ntp-conversa="'+id+'"]').click();await p.waitForFunction(()=>document.getElementById('ntpConversaStatus').textContent==='Carregando conversa…');
  ...
  await p.locator('#ntpConversaFechar').click();await p.locator('[data-ntp-conversa="'+repeated.id+'"]').click();await p.locator('[data-ntp-apaga="'+repeated.id+'"]').click();ok(!await p.locator('#ntpConversa').isVisible(),'Excluir refeição fecha sua conversa imediatamente');eq(await p.locator('#ntpComentarios').textContent(),'','Excluir limpa mensagens do painel aberto');
...
async function testOfflineAndIdentity(){
  ...
  await p.locator('[data-ntp-conversa="'+id+'"]').click();await p.locator('#ntpResposta').fill('Mensagem guardada');await p.locator('#ntpResponder').click();await p.waitForFunction(()=>document.getElementById('ntpConversaStatus').textContent.includes('Primeiro sincronize'));eq(state.calls.filter(x=>x.rpc==='app_nutricao_feedback_envia').length,0,'Não envia conversa para registro ainda não sincronizado');
  await p.reload();await p.waitForFunction(()=>window.__nutriAluno);await open(p);await p.locator('[data-ntp-conversa="'+id+'"]').click();eq(await p.locator('#ntpResposta').inputValue(),'Mensagem guardada','Rascunho de resposta sobrevive ao recarregar');
