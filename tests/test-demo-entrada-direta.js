...
(async()=>{
 b=await chromium.launch({executablePath:process.env.CHROMIUM_PATH||'/opt/pw-browsers/chromium',args:['--no-sandbox']});
 for(const width of [390,1280]) for(const file of ['demo-aluno.html','demo-aluno-sem-cadastro.html','demo-aluno-cadastro.html']){
  ...
  await ctx.route('**/*',r=>{const u=new URL(r.request().url());if(u.origin===new URL(BASE).origin)return r.continue();external.push(u.origin+u.pathname);return r.abort();});
  ...
  ok(external.length===0,label+': nenhuma requisição externa: '+external.join(';'));
