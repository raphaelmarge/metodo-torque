...
async function main() {
  ...
  try {
    ...
    ok(JSON.stringify((await registro(p)).map(r=>[r.serie,r.r,r.kg,r.feito]))===JSON.stringify([[1,5,60,true],[2,8,70,true],[3,10,80,true]]),'três confirmações, com seleção das séries, registram execuções distintas sem digitação');
    ok(await volume(p)===1660,'volume usa apenas os três registros confirmados');

    const igual=await caso(ficha([60,60,60]));
    await preencherRegistro(igual, '#gKg', '42');await clicarControle(igual, '#gSerie');
    ...
    await clicarControle(igual, '#gSerie');
    ...
    await preencherRegistro(diversa, '#gKg', '42');await clicarControle(diversa, '#gSerie');
    ok(await diversa.inputValue('#gKg')==='70','prescrição de carga diferente vence o ajuste da série anterior');
    await clicarControle(diversa, '#gSerie');
    ...
    const zeroManual=await caso(ficha([60,60,60]));await preencherRegistro(zeroManual, '#gKg', '0');await zeroManual.locator('#gKg').blur();
    ok(await zeroManual.inputValue('#gKg')==='0','zero digitado permanece zero após sair do campo');
    await clicarControle(zeroManual, '#gSerie');
    ...
    await fora.click('[data-gserie="0"]');await preencherRegistro(fora, '#gKg', '42');await fora.click('.gserie-ajustes summary');await clicarControle(fora, '#gSalvar');
    await fora.click('[data-gserie="2"]');await preencherRegistro(fora, '#gKg', '90');await clicarControle(fora, '#gSerie');
    await fora.click('[data-gserie="0"]');await clicarControle(fora, '#gSerie');
    ...
    await preencherRegistro(hist, '#gKg', '42');await clicarControle(hist, '#gSerie');
    ...
    await preencherRegistro(draft, '#gKg', '42');await draft.click('[data-gserie="1"]');await draft.click('[data-gserie="0"]');
    ...
    await clicarControle(draft, '#gSerie');
    rows=await registro(draft);ok(rows[0].kg===42&&rows[0].r===5&&rows[0].feito,'Registrar série confirma os valores do draft retomado');
    const soReps=await caso(ficha());await preencherRegistro(soReps, '#gReps', '4');await soReps.click('#gFechar');
