...
(async()=>{
 ...
 ok(!(await p.locator('#marcaMostrarSecundario').isChecked()),'segunda linha desligada por padrão');
 await p.locator('#marcaModo').selectOption('estudio');await p.locator('#marcaEstudio').fill('Studio Horizonte');await p.locator('#marcaProfissional').fill('Ana Silva');await p.locator('#marcaCurto').fill('Horizonte');await p.locator('#marcaSlogan').fill('Movimento com cuidado');
 eq(await p.locator('#marcaPreviewSecundario').textContent(),'','só marca não cria assinatura automática');
 ...
 for(const mode of ['profissional','estudio','ambos']){
  await p.locator('#marcaModo').selectOption(mode==='ambos'?'profissional':mode);
 ...
 await p.locator('#marcaModo').selectOption('estudio');await p.locator('#marcaMostrarSecundario').check();await p.locator('#marcaEstudio').fill('Studio Horizonte');await p.locator('#marcaSalvar').click();
 ...
 if(await student.locator('#tourPular').isVisible())await student.locator('#tourPular').click();
 await student.evaluate(()=>window.scrollTo(0,0));
 // Opt-out após publicação: elimina a linha do DOM, não só a esconde com CSS.
 ...
 // A marca é cabeçalho comercial; assinatura do recibo continua sendo a pessoa.
 ...
 const diario=await p.locator('#pgHoje').textContent();
