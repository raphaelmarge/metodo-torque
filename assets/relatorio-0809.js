...
(function (root) {
  ...
  api.init=function(c){
    ...
    $('cbSalva').addEventListener('click',function(e){try{if($('cbAluno').value)corridaContexto();if(blocoPendente())throw new Error('Adicione ou salve a etapa em edição antes de salvar o treino. Você também pode cancelar a edição da etapa.');}catch(err){e.preventDefault();e.stopImmediatePropagation();msg('cbStatus',err);msg('r809CorridaStatus',err);if(blocoPendente())corridaFoco('r809BlocoAdd');}},true);
    ...
    $('r809BlocoCancel').onclick=function(){blocoLimpa();zonaOptions(tDe(ctx.load(),corridaAluno).zonasCorrida||[]);bsDraw();corridaGuarda();$('r809CorridaStatus').textContent='Edição da etapa cancelada. A sequência foi mantida.';};
    $('r809BlocoAdd').onclick=function(){try{corridaContexto();var b=blocoForm(),bs=bsForm(),edit=!!blocoEdit;if(blocoEdit){if(JSON.stringify(bs)!==blocoEdit.base)throw new Error('A sequência mudou. Cancele e reabra o bloco para conferir.');bs[blocoEdit.index]=b;}else bs.push(b);blocos(bs);blocoLimpa();bsWrite(bs);zonaOptions(tDe(ctx.load(),corridaAluno).zonasCorrida||[]);$('r809CorridaStatus').textContent=(edit?'Etapa atualizada':'Etapa adicionada')+' no rascunho. Salve o treino para aplicar.';}catch(e){msg('r809CorridaStatus',e);}};
