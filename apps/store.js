...
(function () {
  ...
  function enviaSujas() {
    ...
    var linhas = chaves.map(function (k) {
      ...
      return { academia_id: sync.aid, chave: k, valor: valor, atualizado: m[k] || new Date().toISOString(),
        base_atualizado: chaveProtegida(k) ? baseDe(k) : null };
    });
    ...
    sync.promessaEnvio = q.then(function (r) {
      ...
      if (r.error) {
        ...
        if (r.error.code === 'PT409') chaves.filter(chaveProtegida).forEach(function (k) { sinalizaConflito(k, r.error.message); });
        avisaStatus();
      } else {
  ...
  function resolveConflito(k) {
    ...
    if (!sync.client || sync.emEnvio) return Promise.resolve(false);
  ...
  window.MTStore = {
    ...
    iniciaSync: iniciaSync, publicaAppsSeguros: publicaAppsSeguros,
